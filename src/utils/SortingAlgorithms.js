export function mergeSort(items) {
  ...
  results.push(JSON.parse(JSON.stringify(merge_sort(resultItems))));
  return results;
}
...
export function selectionSort(items) {
  ...
  while (current_index < resultItems.length - 1) {
    ...
    [resultItems[current_index], resultItems[min_idx]] = [
      resultItems[min_idx],
      resultItems[current_index],
    ];
    current_index += 1;
    results.push(JSON.parse(JSON.stringify(resultItems)));
  }
...
export function getAlgoFunction(algoFunction) {
  if (algoFunction === "BubbleSort") return bubbleSort;

  if (algoFunction === "InsertionSort") return insertionSort;

  if (algoFunction === "SelectionSort") return selectionSort;

  if (algoFunction === "MergeSort") return mergeSort;

  if (algoFunction === "QuickSort") return quickSort;
}
